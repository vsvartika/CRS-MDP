...
% Randomly choose D_mat
   ...
   for k = 1:1000000
    ...
    if Max_sofar < rew_exp && cost_exp <= B 
       ...
       tmp = rand + 0.2*le(conseq_cnt, 150) + le (k, 1000);
       
       if tmp < 10/k*exp(-00000.1* (conseq_cnt)^(0.3)) 
           ...
           if 1 == 0
           if isempty(d_temp) && conseq_cnt < 3
           ...
           % learning better policy  
            ...
            if  isempty(d_temp) 
                ...
                eps_policy = min(0.01, 1.5/conseq_cnt^(.95));
                ofs = 0;
                for t=1:T-1 
                    temp = d_temp(ofs+(1:N_s*N_a));
                    ofs= ofs+ N_s*N_a;
       ...
       if conseq_cnt > 200
           ...
           if mod(conseq_cnt, 200) == 1
              %  PrintD_mat (D_mat, N_s, N_a, T,k)
                ...
                str = sprintf (' D_mat \\gamma = %-3.2f T = %d B = %-3.2f', gam, T, log(B)/gamma);
                title (str);
                ...
                str = sprintf (' MaxD_mat \\gamma = %-3.2f T = %d B = %-3.2f', gam, T, log(B)/gamma);
                title (str);
